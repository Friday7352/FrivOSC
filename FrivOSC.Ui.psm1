...
function Get-FrivoTheme {
    ...
    return [pscustomobject] @{
        ...
        # Frivo's red (#fa2f2f), not Evora's purple: FrivOSC is a part of
        # Frivo rather than a product of its own, and the accent is what
        # says so before anyone reads a word of the window.
        Accent  = [System.Drawing.Color]::FromArgb(250, 47, 47)      # --accent
        AccentH = [System.Drawing.Color]::FromArgb(255, 92, 92)
        AccentP = [System.Drawing.Color]::FromArgb(198, 28, 28)
...
function New-FrivoForm {
    ...
    if ($IconPath -and (Test-Path -LiteralPath $IconPath)) {
        ...
        # does.  Windows then uses the icon from the real FrivOSC shortcut
        # instead of creating a separate, generic PowerShell taskbar group.
        Set-FrivoTaskbarIcon -Form $f -IconPath $IconPath -AppId 'FrivOSC.Setup'
    }
    return $f
}

...
function Get-FrivOSCVersion {
    <#
        The one place the version is written down. Everything else reads
        from here: the Apps & features entry, the setup wizard, the Inno
        container, this exe's file properties, and the service itself.
    #>
    $path = Join-Path $root 'VERSION'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw ("VERSION was not found at {0}. Create it with a version number, e.g. 1.1.0" -f $path)
    }
    $version = (Get-Content -LiteralPath $path -Raw).Trim()
    # Checked here because csc and Inno both reject a malformed version
    # later, with errors that say nothing about this file.
    if ($version -notmatch '^\d+(\.\d+){1,3}$') {
        throw ("VERSION contains '{0}', which is not a version number. Use digits and dots." -f $version)
    }
    return $version
}

function New-FrivOSCVersionSource {
    param([string] $Version, [string] $Path)

    # .NET wants four parts. Padding explicitly rather than letting it
    # happen silently, so what Explorer shows matches what is written here.
    $parts = @($Version -split '\.')
    while ($parts.Count -lt 4) { $parts += '0' }
    $assemblyVersion = ($parts[0..3] -join '.')

    Set-Content -LiteralPath $Path -Encoding UTF8 -Value @(
        '// Generated by Build-FrivOSCInstaller.ps1 from the VERSION file at',
        '// the repo root. Do not edit: the next build overwrites it.',
...
function Build-FrivOSCHost {
    ...
    $versionSource = Join-Path $build 'Version.generated.cs'
    New-FrivOSCVersionSource -Version (Get-FrivOSCVersion) -Path $versionSource
...
$version = Get-FrivOSCVersion
Write-Host ('Building FrivOSC {0}' -f $version)
